...
class SubCategoriasController extends ControladorBase{
		...
		if (isset(  $_SESSION['usuario_usuario']) )
		{
		  ...
		  //Creamos el objeto usuario
			...
			if (!empty($resultPer))
			{
				...
				$columnas = " subcategorias.id_subcategorias, categorias.id_categorias, categorias.nombre_categorias, subcategorias.nombre_subcategorias, subcategorias.path_subcategorias, subcategorias.lecturas_subcategorias, subcategorias.total_pdf_subcategorias, subcategorias.total_xml_subcategorias, subcategorias.subidos_xml_subcategorias ";
	...
	public function ReporteTotal1(){
		...
		$columnas = " subcategorias.nombre_subcategorias, COUNT(documentos_legal.paginas_documentos_legal) AS lecturas_documentos, SUM(documentos_legal.paginas_documentos_legal)  AS paginas_documentos";
		$tablas   = " public.subcategorias, public.documentos_legal";
		$where    = " subcategorias.id_subcategorias = documentos_legal.id_subcategorias AND etapa_documentos_legal = 1 GROUP BY subcategorias.nombre_subcategorias";
		$id       = " subcategorias.nombre_subcategorias";
	
	
		$columnas2 = " 'TOTALES' AS totales,  SUM(paginas_documentos_legal) AS total_paginas, COUNT(id_documentos_legal) AS total_documentos";
		$where2 = "id_documentos_legal > 0 AND etapa_documentos_legal = 1 ";
	...
	public function ReporteTotal2(){
		...
		$columnas = " subcategorias.nombre_subcategorias, COUNT(documentos_legal.paginas_documentos_legal) AS lecturas_documentos, SUM(documentos_legal.paginas_documentos_legal)  AS paginas_documentos";
		$tablas   = " public.subcategorias, public.documentos_legal";
		$where    = " subcategorias.id_subcategorias = documentos_legal.id_subcategorias AND etapa_documentos_legal = 2 GROUP BY subcategorias.nombre_subcategorias";
		$id       = " subcategorias.nombre_subcategorias";
	
	
		$columnas2 = " 'TOTALES' AS totales,  SUM(paginas_documentos_legal) AS total_paginas, COUNT(id_documentos_legal) AS total_documentos";
		$where2 = "id_documentos_legal > 0 AND etapa_documentos_legal = 2 ";
